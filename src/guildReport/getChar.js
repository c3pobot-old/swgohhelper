...
module.exports = async(uInfo = {}, homeUnits = [], awayUnits)=>{
  try{
    ...
    const homeRelic = await CalcUnitRelic(homeUnits)
    const awayRelic = await CalcUnitRelic(awayUnits)
    if(uInfo.skills){
       omiSkills = Object.values(uInfo.skills).filter(omiFilter[omiOpt])
       if(Object.values(uInfo.skills).filter(x=>x.zetaTier).length > 0){
         homeZeta = await CalcUnitZeta(uInfo.skills, homeUnits)
         awayZeta = await CalcUnitZeta(uInfo.skills, awayUnits)
       }
    }
    if(uInfo.ultimate && Object.values(uInfo.ultimate).length > 0){
      homeUlt = await CalcUnitUlt(homeUnits)
      awayUlt = await CalcUnitUlt(awayUnits)
    }
    if(omiSkills?.length > 0){
      homeOmi = await CalcUnitOmi(omiSkills, homeUnits)
      awayOmi = await CalcUnitOmi(omiSkills, awayUnits)
    }
    if(homeRelic){
