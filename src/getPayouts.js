...
module.exports = async(obj = {}, players = [], auto = false)=>{
  try{
    ...
    if(players?.length > 0){
      ...
      array = await sorter([{column: 'payoutTime', order: obj.poSort}], Object.values(pObj))
    }
    if(array?.length > 0){
      const msgFields = []
      for(let i in array){
        ...
        const playerArray = await sorter([{column: obj.sort, order: 'ascending'}], array[i].players)
