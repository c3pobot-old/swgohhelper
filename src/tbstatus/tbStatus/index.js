...
module.exports = async(guildId, guildData)=>{
  try{
    ...
    const tbDef = (await mongo.find('tbDefinition', {_id: guildData.definitionId}))[0]
    const getPscores = (await mongo.find('tbCache', {_id: guildId}, {_id:0, pScores: 1}))[0]
    ...
    const tbData = await NewObj(guildData)
    ...
    for(let i in guildData.conflictZoneStatus){
      ...
      if(guildData.conflictZoneStatus[i].zoneStatus.zoneState === 3){
        ...
        for(let c in cmInfo){
          ...
          const cmScore = +guildData.strikeZoneStatus.find(x=>x.zoneStatus.zoneId === cmInfo[c].zoneDefinition.zoneId)?.zoneStatus?.score || 0
          const cmCount = +guildData.strikeZoneStatus.find(x=>x.zoneStatus.zoneId === cmInfo[c].zoneDefinition.zoneId).playersParticipated || 0
        ...
        let smTotalScore = 0
        for(let c in smInfo){
          const lastSMScore = (pScore.sm.find(x=>x.zoneId === smInfo[c].zoneDefinition.zoneId) ? pScore.sm.find(x=>x.zoneId === smInfo[c].zoneDefinition.zoneId).score : 0)
          newpScore.sm.push({
    ...
    tbData.gpUndeployed = await GetTotalUndeployed(tbData.gp, guildData.currentStat?.find(x=>x.mapStatId === 'power_round_'+currentRound)?.playerStat)
    tbData.gpCharUndepoyed = tbData.gpCharUndepoyed
    tbData.gpShipUndeployed = tbData.gpShipUndeployed
    tbData.gp = tbData.gp
    tbData.zoneData = await sorter([{column: 'sort', order: 'ascending'}], tbData.zoneData)
