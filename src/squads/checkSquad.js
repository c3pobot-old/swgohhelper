...
module.exports = async(squad = {}, pRoster = [], ignoreStats = false)=>{
  try{
    ...
    for(let i in squad.units){
      ...
      const tempObj = await FT.FormatWebUnit(squadUnit, uInfo)
      tempObj.notMet = 0
      ...
      if(squadUnit){
        ...
        if(!ignoreStats && squad.units[i].stats && squad.units[i].stats.length > 0){
          info.showStats = true
          const checkStats = await CheckStats(squad.units[i].stats, squadUnit.stats, squad.units[i].combatType)
      ...
      if(tempObj.reqRarity && tempObj.reqRarity > 1) await AddRequirement(tempObj, tempObj.reqRarity.toString(), '*')
      if(tempObj.reqRelic){
        await AddRequirement(tempObj, 'R', (+tempObj.reqRelic - 2).toString())
      }else{
        if(tempObj.reqGear) await AddRequirement(tempObj, 'G', tempObj.reqGear.toString())
      }
      units.push(tempObj)
    }
    if(squad.units.filter(x=>x.turn).length > 0){
      await CheckTurnOrder(units, squadUnits, squad)
    }
