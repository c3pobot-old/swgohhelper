...
const GetRelicMats = async(unit = {}, relicRecipe = [], res = {}, inventory = [])=>{
  try{
    let neededRelicMats = await GetNeededRelicMats([], relicRecipe.filter(x=>(unit.reqRelic - 2) >= x.tier), (unit.relic - 2) || 0, unit.reqRelic - 2)
...
module.exports = async(units = [], relicRecipe = [], inventory)=>{
  try{
    ...
    for(let i in units){
      ...
      if(units[i].reqRelic > 2 && units[i].reqRelic > units[i].relic) await GetRelicMats( units[i], relicRecipe, res.relicMats, inventory?.material )
